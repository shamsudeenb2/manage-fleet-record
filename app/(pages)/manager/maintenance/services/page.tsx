...
const STATUS_STYLES: Record<string, string> = {
  SCHEDULED:   "bg-[#B8860B]/20 text-[#B8860B] border border-[#B8860B]/30",
...
function DuePill({ date, km, currentOdo }: { date?: string | null; km?: number | null; currentOdo?: number | null }) {
  ...
  if (date) {
    ...
    else                 parts.push(<span key="d" className="text-[#9C9590] text-[10px]">{fmt(date)}</span>);
  }
  if (km) parts.push(<span key="k" className="text-[#9C9590] text-[10px]">{km.toLocaleString("en-NG")} km</span>);
  return parts.length > 0 ? <div className="flex flex-col gap-0.5">{parts}</div> : <span className="text-[#9C9590] text-[10px]">—</span>;
}
      ...
      <div className="min-h-screen bg-[#F8F6F1] text-[#1C1917]" style={{ fontFamily: "'DM Mono','Fira Mono',monospace" }}>
        <div className="border-b border-[#E8E2D9] bg-[#F8F6F1]/80 backdrop-blur sticky top-0 z-30">
          <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
            <div><h1 className="text-base font-bold tracking-wider">Services</h1><p className="text-xs text-[#9C9590] mt-0.5">{total} service records</p></div>
            <button onClick={() => router.push("/maintenance/services/create")} className="px-4 py-2 rounded-lg text-xs bg-[#B8860B] text-[#1C1917] font-bold hover:bg-[#C9960D] transition-colors">+ Log Service</button>
          </div>
          {overdueServices > 0 && (
            <div className="max-w-7xl mx-auto px-6 pb-3">
              <div className="flex items-center gap-3 bg-red-50 border border-red-700/30 rounded-lg px-4 py-2">
          ...
          <div className="max-w-7xl mx-auto px-6 pb-3 flex items-center gap-3 flex-wrap">
            <div className="relative flex-1 max-w-xs">
              ...
              <input type="text" value={q} onChange={e => setQ(e.target.value)} placeholder="Search vehicle, garage, description…" className="w-full bg-white border border-[#E8E2D9] rounded-lg pl-7 pr-7 py-1.5 text-xs text-[#2C2825] placeholder-[#B0AAA4] focus:outline-none focus:border-[#B8860B]/40" />
        ...
        <div className="max-w-7xl mx-auto px-6 py-5 space-y-5">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              { label: "Scheduled",       value: scheduled,     icon: "◷",  accent: "#B8860B", alert: scheduled > 0 },
          ...
          <div className="bg-white border border-[#E8E2D9] rounded-xl overflow-hidden">
            <div className="px-5 py-3 border-b border-[#E8E2D9] flex items-center justify-between">
              <span className="text-xs font-semibold uppercase tracking-widest text-[#6B6560]">{items.length} service{items.length !== 1 ? "s" : ""}</span>
                    ...
                    {items.map((svc, i) => (
                      <motion.tr key={svc.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ delay: i * 0.015 }}
                        ...
                        <td className="py-3.5 px-5">
                          {svc.vehicle ? <button onClick={() => router.push(`/vehicles/${svc.vehicle!.id}`)} className="text-left hover:text-[#B8860B] transition-colors"><div className="font-mono text-[#B8860B]">{svc.vehicle.plateNumber}</div><div className="text-[#9C9590] text-[10px]">{svc.vehicle.cap_no}</div></button> : <span className="text-[#9C9590]">—</span>}
                          {svc.driver && <div className="text-[#9C9590] text-[10px] mt-0.5">{svc.driver.name}</div>}
                        </td>
                        <td className="py-3.5 px-5"><div className="text-[#2C2825]">{TYPE_LABELS[svc.serviceType] ?? svc.serviceType}</div>{svc.description && <div className="text-[#9C9590] text-[10px] mt-0.5 truncate max-w-[180px]">{svc.description}</div>}</td>
                        ...
                        <td className="py-3.5 px-5 font-mono text-[#2C2825]">{svc.totalCost ? fmtCost(svc.totalCost) : "—"}</td>
                        <td className="py-3.5 px-5"><DuePill date={svc.nextServiceDate} km={svc.nextServiceKm} /></td>
                        <td className="py-3.5 px-5 text-[#9C9590] whitespace-nowrap">{svc.scheduledDate ? fmt(svc.scheduledDate) : "—"}</td>
                        <td className="py-3.5 px-5"><div className="flex items-center gap-2">
                          {svc.status==="COMPLETED"?(<></>):(<button onClick={() => router.push(`/maintenance/services/${svc.id}`)} className="px-2 py-1 rounded text-xs border border-[#B8860B]/30 text-[#B8860B] hover:bg-[#B8860B]/10 transition-colors">Edit</button>)}
                        <ConfirmDeleteButton onConfirm={() => deleteService(svc.id)} /></div></td>
                      </motion.tr>
                    ))}
                  </AnimatePresence>
                  {!loading && items.length === 0 && <tr><td colSpan={7} className="py-16 text-center text-[#9C9590]">{q ? `No services matching "${q}"` : "No services logged yet"}</td></tr>}
                </tbody>
              </table>
            </div>
            {totalPages > 1 && (
              <div className="px-5 py-3 border-t border-[#E8E2D9] flex items-center justify-between">
                <span className="text-xs text-[#9C9590] font-mono">Page {page} of {totalPages} · {total} services</span>
