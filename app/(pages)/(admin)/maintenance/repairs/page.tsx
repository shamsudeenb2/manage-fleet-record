...
const STATUS_STYLES: Record<string, string> = {
  OPEN:        "bg-red-50 text-red-400 border border-red-800/30",
  ...
  CANCELLED:   "bg-zinc-700/30 text-[#6B6560] border border-zinc-600/30",
};

const PRIORITY_STYLES: Record<string, string> = {
  LOW:      "bg-zinc-700/30 text-[#6B6560] border border-zinc-600/30",
  MEDIUM:   "bg-[#3E6B8C]/20 text-sky-400 border border-[#3E6B8C]/30",
  HIGH:     "bg-amber-50 text-amber-400 border border-amber-700/30",
      ...
      <div className="min-h-screen bg-[#F8F6F1] text-[#1C1917]" style={{ fontFamily: "'DM Mono','Fira Mono',monospace" }}>
        <div className="border-b border-[#E8E2D9] bg-[#F8F6F1]/80 backdrop-blur sticky top-0 z-30">
          <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
            <div><h1 className="text-base font-bold tracking-wider">Repairs</h1><p className="text-xs text-[#9C9590] mt-0.5">{total} repair records</p></div>
            <button onClick={() => router.push("/maintenance/repairs/create")} className="px-4 py-2 rounded-lg text-xs bg-[#B8860B] text-[#1C1917] font-bold hover:bg-[#C9960D] transition-colors">+ Log Repair</button>
          </div>
          {criticalCount > 0 && (
          ...
          <div className="max-w-7xl mx-auto px-6 pb-3 flex items-center gap-3 flex-wrap">
            <div className="relative flex-1 max-w-xs">
              ...
              <input type="text" value={q} onChange={e => setQ(e.target.value)} placeholder="Search fault, vehicle, garage…" className="w-full bg-white border border-[#E8E2D9] rounded-lg pl-7 pr-7 py-1.5 text-xs text-[#2C2825] placeholder-[#B0AAA4] focus:outline-none focus:border-[#B8860B]/40" />
            ...
            {["ALL", "LOW", "MEDIUM", "HIGH", "CRITICAL"].map(p => (
              <button key={p} onClick={() => { setPriority(p); setPage(1); }}
                className={`px-2.5 py-1 rounded-lg text-[10px] font-semibold uppercase tracking-wider transition-all ${priority === p ? "bg-[#B8860B] text-[#1C1917]" : "border border-[#E8E2D9] text-[#9C9590] hover:text-[#2C2825]"}`}>
        ...
        <div className="max-w-7xl mx-auto px-6 py-5 space-y-5">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <motion.div key={s.label} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                className={`relative bg-white border rounded-xl p-4 overflow-hidden ${s.alert ? "border-red-700/40" : "border-[#E8E2D9]"}`}>
          ...
          <div className="bg-white border border-[#E8E2D9] rounded-xl overflow-hidden">
            <div className="px-5 py-3 border-b border-[#E8E2D9] flex items-center justify-between">
              <span className="text-xs font-semibold uppercase tracking-widest text-[#6B6560]">{items.length} repair{items.length !== 1 ? "s" : ""}</span>
                    ...
                    {items.map((repair, i) => (
                      <motion.tr key={repair.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ delay: i * 0.015 }}
                        ...
                        <td className="py-3.5 px-5">
                          {repair.vehicle ? <button onClick={() => router.push(`/vehicles/${repair.vehicle!.id}`)} className="text-left hover:text-[#B8860B] transition-colors"><div className="font-mono text-[#B8860B]">{repair.vehicle.plateNumber}</div><div className="text-[#9C9590] text-[10px]">{repair.vehicle.cap_no}</div></button> : <span className="text-[#9C9590]">—</span>}
                          {repair.driver && <div className="text-[#9C9590] text-[10px] mt-0.5">{repair.driver.name}</div>}
                        </td>
                        <td className="py-3.5 px-5 max-w-[200px]"><div className="text-[#2C2825] truncate">{repair.faultDesc}</div>{repair.parts && repair.parts.length > 0 && <div className="text-[#9C9590] text-[10px] mt-0.5">{repair.parts.length} part{repair.parts.length > 1 ? "s" : ""}</div>}</td>
                        ...
                        <td className="py-3.5 px-5 font-mono text-[#2C2825]">{repair.totalCost ? fmtCost(repair.totalCost) : "—"}</td>
                        <td className="py-3.5 px-5 text-[#9C9590] truncate max-w-[120px]">{repair.garage ?? "—"}</td>
                        <td className="py-3.5 px-5 text-[#9C9590] whitespace-nowrap">{fmt(repair.reportedDate)}</td>
                        <td className="py-3.5 px-5"><div className="flex items-center gap-2">
                          <button onClick={() => router.push(`/maintenance/repairs/${repair.id}`)} className="px-2 py-1 rounded text-xs border border-[#B8860B]/30 text-[#B8860B] hover:bg-[#B8860B]/10 transition-colors">Edit</button>
                        <ConfirmDeleteButton onConfirm={() => deleteRepair(repair.id)} /></div></td>
                      </motion.tr>
                    ))}
                  </AnimatePresence>
                  {!loading && items.length === 0 && <tr><td colSpan={8} className="py-16 text-center text-[#9C9590]">{q ? `No repairs matching "${q}"` : "No repairs logged yet"}</td></tr>}
                </tbody>
              </table>
            </div>
            {totalPages > 1 && (
              <div className="px-5 py-3 border-t border-[#E8E2D9] flex items-center justify-between">
                <span className="text-xs text-[#9C9590] font-mono">Page {page} of {totalPages} · {total} repairs</span>
