...
// import { computePartsCost } from "@/lib/cost-utils";

// export async function POST(req, { params }) {
//   try {
//     const repairId = params.repairId;
//     const { partId, qtyUsed } = await req.json();

//     const part = await prisma.part.findUnique({ where: { id: partId } });

//     if (!part) return NextResponse.json({ error: "Part not found" }, { status: 404 });

//     const cost = computePartsCost(qtyUsed, part.unitPrice);

//     await prisma.partUsedInRepair.create({
//       data: {
//         partId,
//         repairId,
//         qtyUsed,
...
//     // recompute total repair part cost
//     const items = await prisma.partUsedInRepair.findMany({
//       where: { repairId },
//       include: { part: true },
//     });

//     const totalCost = items.reduce(
//       (sum, item) => sum + (item.qtyUsed * item.part.unitPrice),
//       0
//     );
...
//     return NextResponse.json({ message: "Part added", totalCost });
//   } catch (e) {
//     console.error(e);
