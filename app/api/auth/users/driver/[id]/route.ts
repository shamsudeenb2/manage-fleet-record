...
//     // ── All trips for this driver (no date filter) ────────────────
//     const allTrips = await prisma.trip.findMany({
//       where: { driverId: id },
//       include: { fuels: true, vehicle: true },
//       orderBy: { despatchDate: "desc" },
//     });

//     // ── Trips in date range ───────────────────────────────────────
//     const filteredTrips = await prisma.trip.findMany({
//       where: { driverId: id, ...dateFilter },
//       include: { fuels: true, vehicle: true },
//       orderBy: { despatchDate: "desc" },
//     });
...
//     // ── Avg fuel efficiency across vehicles driven (range) ────────
//     const fuelEfficiency =
//       totalFuelConsume > 0 ? totalDistanceInRange / totalFuelConsume : 0;
...
//     // ── Vehicle history from TruckDriver ──────────────────────────
...
//     const vehicleHistory = truckDriverHistory.map((td) => {
//       const from = new Date(td.from);
//       const to = new Date(td.to);
//       const days = Math.ceil((to.getTime() - from.getTime()) / (1000 * 60 * 60 * 24));
//       // compute trips on this vehicle during this assignment window
//       const tripsOnVehicle = allTrips.filter(
//         (t) =>
//           t.vehicleId === td.vehicleId &&
...
//         plateNumber: td.vehicle?.plateNumber ?? "—",
//         cap_no: td.vehicle?.cap_no ?? "—",
//         make: td.vehicle?.make ?? null,
//         model: td.vehicle?.model ?? null,
//         year: td.vehicle?.year ?? null,
//         fuelType: td.vehicle?.fuelType ?? null,
//         vehicleImg: td.vehicle?.vehicleImg ?? null,
//         from: td.from,
//         to: td.to,
//         daysAssigned: days,
//         tripsOnVehicle,
...
//     const uniqueVehicleIds = [...new Set(truckDriverHistory.map((td) => td.vehicleId))];

//     // ── Parts replaced on all vehicles this driver drove ─────────
//     // Parts belonging to vehicles this driver has driven
//     const parts = await prisma.part.findMany({
//       where: { vehicleId: { in: uniqueVehicleIds } },
//       include: {
//         vehicle: { select: { plateNumber: true, cap_no: true, currentOdo: true } },
//         usedInRepair: {
//           include: {
...
//       const currentOdo = p.vehicle?.currentOdo ?? 0;
//       const kmOnVehicle = installOdo != null ? currentOdo - installOdo : null;
...
//     // ── Repairs & Services across all vehicles ────────────────────
...
//     // ── Recent trips ──────────────────────────────────────────────
//     const recentTrips = filteredTrips.slice(0, 15).map((t) => ({
//       id: t.id,
...
//     // ── Vehicle distribution (trips per vehicle, all time) ────────
//     const vehicleTripMap: Record<string, { plate: string; trips: number }> = {};
//     for (const t of allTrips) {
//       if (!vehicleTripMap[t.vehicleId]) {
//         vehicleTripMap[t.vehicleId] = { plate: t.vehicle?.plateNumber ?? t.vehicleId, trips: 0 };
//       }
//       vehicleTripMap[t.vehicleId].trips++;
//     }
//     const vehicleDistribution = Object.values(vehicleTripMap)
//       .sort((a, b) => b.trips - a.trips)
//       .map(({ plate, trips }) => ({ name: plate, value: trips }));
...
//     let licenseStatus: "valid" | "expiring_soon" | "expired" | "unknown" = "unknown";
//     let daysUntilLicenseExpiry: number | null = null;
//     if (driver.licenseExp) {
//       const diffMs = new Date(driver.licenseExp).getTime() - Date.now();
//       daysUntilLicenseExpiry = Math.ceil(diffMs / (1000 * 60 * 60 * 24));
//       if (daysUntilLicenseExpiry < 0) licenseStatus = "expired";
//       else if (daysUntilLicenseExpiry <= 30) licenseStatus = "expiring_soon";
...
//     });
//   } catch (err: any) {
//     console.error("[driver-details]", err);
//     return NextResponse.json({ message: err?.message ?? "Internal error" }, { status: 500 });
//   }
// }

// src/app/api/drivers/details/[id]/route.ts
//
// Full driver dashboard data endpoint.
//
// TRIP RANGE  (from / to)   → controls charts, KPI totals, recent trips
// MAINT RANGE (mFrom / mTo) → controls maintenance tables only (independent)
//
// MAINTENANCE ENRICHMENT (per record):
//   vehicleId / vehiclePlate / vehicleCapNo
//     → the truck that was being maintained (clickable link target)
//
//   reporterDriverId / reporterDriverName
//     → the driver FK stored on the record (repairs.driverId / services.driverId)
//       i.e. the person who REPORTED or SUPERVISED the job.
//       For parts  → comes from the linked Repair's driverId (no direct FK on Part).
//       For tires  → inferred from the nearest trip on the vehicle ±7 days of fittedDate.
//
//   kmDrivenFromEvent
//     → total km THIS dashboard-driver drove on THAT vehicle
//       FROM the event date (fittedDate / reportedDate / scheduledDate / purchaseDate)
//       TO the mTo filter end (or today).
//       Rationale: shows how much the driver used the vehicle AFTER the maintenance
//       event — useful for warranty/wear analysis.
...
//     // handle vehicle assignment/unassignment in transaction

//     // const updated = await prisma.$transaction(async (tx) => {
//     //   if ("vehicle" in data) {
//     //     // unassign old vehicle from driver
//     //     const current = await tx.driver.findUnique({ where: { id }, select: { vehicle: true } });
//     //     if (!current) throw new Error("Driver not found");

//     //     if (current.vehicle) {
//     //       await tx.vehicle.update({ where: { id: current.vehicle }, data: { driverId: null } });
//     //     }
//     //     // assign new vehicleId if present
//     //     if (data.vehicleId) {
//     //       const v = await tx.vehicle.findUnique({ where: { id: data.vehicleId } });
//     //       if (!v) throw new Error("Vehicle not found");
//     //       if (v.driverId && v.driverId !== id) throw new Error("Vehicle already assigned");
//     //       await tx.vehicle.update({ where: { id: data.vehicleId }, data: { driverId: id } });
//     //     }
//     //   }

//     //   // update driver fields (excluding vehicleId handled above)
//     //   const toUpdate = { ...data };
//     //   delete (toUpdate as any).vehicleId;
//     //   const u = await tx.driver.update({ where: { id }, data: toUpdate, include: { vehicle: true } });
//     //   return u;
//     // });

//     const dataToUpdate: any = { ...parsed.data };
//     const u = await prisma.driver.update({ where: { id }, data: dataToUpdate});
